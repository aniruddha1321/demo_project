...
class Calculator:
    def calculate_gpa(self, student: Student) -> float:
        ...
        return self._convert_to_gpa(average)

    def needs_improvement(self, student: Student) -> list:
        improvement_needed = []
        for subject, score in student.grades.items():
            if score < 70:
                improvement_needed.append((subject, score))
        return improvement_needed

    def _convert_to_gpa(self, average: float) -> float:
        if average >= 97:
            return 4.0
        elif average >= 93:
            return 4.0
...
class ReportGenerator:
    def generate_report(self, student: Student, calculator: Calculator) -> str:
        ...
        for subject in sorted(student.grades.keys()):
            ...
            report += f"  %-12s  %6.1f  %5s  {bar}\n" % (subject, score, letter)
